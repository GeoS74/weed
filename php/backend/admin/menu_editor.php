...
{
	/*	на frontend-е если чекбокс не активен класс FormData не будет передавать его ключ
	*	поэтому необходимо устанавливать значение чекбоксов вручную
	*/
	//@Override
	public function set()
	{
		...
		if(is_array($args[0]))
		{
			$args[0]['limited'] = $args[0]['limited'] ? 1 : 0;
			$args[0]['hidden'] = $args[0]['hidden'] ? 1 : 0;
		}
		return parent::set($args[0], $args[1]);
	}
	...
	protected function propProcessing($key, $val='')
	{
		...
		{
			case 'parent_id':
			case 'pos': $val = (int)$val;
				break;
			case 'alias': $val = mb_strpos($val, BASE) === false ? BASE.$val : $val; //автоисправление URL
				break;
		}
